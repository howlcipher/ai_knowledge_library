...
| 6 | [Include Paths Relative to File](#6-include-paths-relative-to-file) | Pending | 6.0 (6×1÷1) | Sonnet 3.5 | Gemini 1.5 Pro | `(include "file.zero")` uses current working directory rather than resolving paths relative to the file doing the inclusion. |
| 7 | [defun Typing Rigidness](#7-defun-typing-rigidness) | Pending | 1.75 (7×1÷4) | Sonnet 3.5 | Gemini 1.5 Pro | All `defun` arguments strictly compile to Go `string` types, breaking if we try to pass an `*http.Request` or `int` to a function. |
