...
- **Close the skill routing recall gap for security prompts (done 2026-07-18):** all 38 skills now declare `triggers` frontmatter; the failing prompt ("security hardening runbook for a FastAPI webhook server") now routes `cyber_security` deterministically. Still worth calibrating `skill_router.score_threshold` against a small labeled prompt set.
...
- **`tier` frontmatter for skills (partially done 2026-07-18):** every SKILL.md now declares a priority `tier:` (0 meta, 1 governance, 2 domain, 3 application), parsed by `SkillRouter` and exposed in `skills.json` and the AGENTS.md manifest (see `documentation/skill_refinement_progress.md`). Still open: pipeline pass affinity for the dispatcher, which should use a distinct key (e.g. `pipeline_pass:`) so the two tier semantics do not collide.
