...
* **Anti-Poisoning & Grounding Protocol:** The AI is mathematically grounded in local rule files (like `GEMINI.md`) and forced to apply an epistemic humility decision tree. If live data conflicts with stale data, or if an action violates the `anti_manipulation.md` constraints, the Orchestrator will automatically halt. 
