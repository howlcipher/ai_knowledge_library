...
## Related Skills
- Defer to `network_engineering` for network segmentation, device configuration, and network telemetry.
- Defer to `cyber_security` for identity governance, secret management, and audit baselines.
- Defer to `devops` for deployment pipelines consuming these hosts.
